...
- Bound metadata-cache entries to a freshly measured extended-attribute signature so APFS security provenance changes cannot reuse an older exact-metadata result; conflicting provenance is preserved as a source-specific recovery variant before cleanup receipts are emitted.
