...
- Added bounded two-worker repository-group processing with disjoint-path auditing, deterministic result collection, exact receipt rehashing, and a mandatory coordinator-only barrier before retirement. Overlapping source or destination trees automatically fall back to one worker.
