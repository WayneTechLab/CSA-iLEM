...
Exact-metadata cache entries also bind a freshly measured extended-attribute signature. If APFS security provenance changes while sources are being reconciled, recovery re-evaluates the file and preserves the alternate exact metadata in that source's recovery variant before any cleanup proof can finalize.
