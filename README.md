...
For a large reviewed inventory, `--group-workers 2` enables bounded repository-level parallelism. Each worker owns one disjoint canonical repository, staging tree, rollback lane, report lane, receipt, and checkpoint; sources targeting the same repository remain serialized. The helper automatically falls back to one worker if source or destination paths overlap. Every worker is joined and every final receipt is rehashed before the single coordinator may begin global retirement. Values above two are rejected to avoid checksum and metadata contention on one external volume.
